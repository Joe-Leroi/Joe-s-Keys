...
if InterfaceV == "Oui" then
		if Auteur == "Joe Leroi & enzoFR60" then
			if ( ply:InVehicle() ) then return end

		if trace.Entity:IsVehicle() and eyepos:DistToSqr(trace.Entity:NearestPoint(eyepos)) < 1000000 then
			if ply:GetNWString("Unlockdo") == "Yes" then
				draw.RoundedBox(0, ScrW()/2.35, ScrH()/2, 190, 35, Color(40, 40, 40, 255 ))
				draw.DrawText( "Véhicule déverrouillé", "Trebuchet24", ScrW()/2, ScrH()/2+2, Color( 255, 255, 255 ), TEXT_ALIGN_CENTER)
			elseif ply:GetNWString("Unlockdo") == "No" then
				draw.RoundedBox(0, ScrW()/2.35, ScrH()/2, 190, 35, Color(40, 40, 40, 255 ))
				draw.DrawText( "Véhicule verrouillé", "Trebuchet24", ScrW()/2, ScrH()/2+2, Color( 255, 255, 255 ), TEXT_ALIGN_CENTER)
			end
		end
	end
	end
